...
def main():
    parser = argparse.ArgumentParser(description="Either simulate and save results or just load results. Then plot.")
    parser.add_argument("--load", action="store_true", help="load from FILE instead of running the simulation.")
    parser.add_argument(
        "--file", default="results.pkl", help="Results file to store to / load from. Default: `results.pkl` (in CWD)."
    )
    parser.add_argument(
        "-o", default="heisenberg_tebd.pdf", help="Output file for the plot. Default `heisenberg_tebd.pdf` (in CWD)"
    )
...
def run_at_fixed_chi(chi):
    ...
    return dict(t=t, S=S, mag_z=mag_z, imbalance=0.5 * np.average(mag_z[:, ::2] - mag_z[:, 1::2], axis=1), err=err)
...
def plot(results, outfile):
    ...
    assert len(chis) == 4
    plot_styles = {
        (chi, which): dict(color=c, label=rf"$\chi={chi}$", ls=ls, lw=2 if chi == max_chi else 1.5)
        for chi, c, ls in zip(chis, ["tab:blue", "tab:orange", "tab:green", "tab:red"], [":", "--", "-.", "-"])
