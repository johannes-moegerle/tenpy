...
class Sweep(Algorithm):
    ...
    def mixer_deactivate(self):
        ...
        """
        logger.info(f"deactivate {self.mixer.__class__.__name__} with final amplitude " f"{self.mixer.amplitude}")
        self.mixer = None
    ...
    def mixer_cleanup(self):
        ...
        for i in range(first, self.psi.L):  # converting S to the left of site i
            ...
            if form_R == 0.0:  # B or Gamma to the right
                ...
                msg = f"Array S are only supported in A, B, Th or G form. " f"Got form {self.psi.form[i]} on site {i}."
                raise RuntimeError(msg)
...
class IterativeSweeps(Sweep):
    ...
    def stopping_criterion(self, iteration_start_time: float) -> bool:
        ...
        if iteration_start_time - self.time0 > max_seconds:
            self.shelve = True
            logger.warning(f"{self.__class__.__name__}: maximum time limit reached. " f"Shelve simulation.")
            return True
        return False
    ...
    def is_converged(self):
        if self.sweeps >= self._min_sweeps and self._tol_theta_diff is not None:
            ...
            if max_diff < self._tol_theta_diff:
                logger.debug(
                    f"VariationalCompression converged after {self.sweeps} sweeps " f"with theta_diff={max_diff}"
                )
                return True
        return False
