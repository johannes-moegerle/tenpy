...
def test_XXZChain():
    pars = dict(L=4, Jxx=1.0, Jz=1.0, hz=0.0, bc_MPS="finite", sort_charge=True)
    chain = XXZChain(pars)
    chain.test_sanity()
...
def test_XXZChain_general(tol=1.0e-14):
    check_general_model(
        XXZChain,
        dict(L=4, Jxx=1.0, hz=0.0, bc_MPS="finite", sort_charge=True),
        {"Jz": [0.0, 1.0, 2.0], "hz": [0.0, 0.2]},
    )
    check_general_model(
        XXZChain2,
        dict(L=4, Jxx=1.0, hz=0.0, bc_MPS="finite", sort_charge=True),
