...
def test_bosonic_model_TEBD():
    ...
    assert abs(estimate - exact) < 1e-10, "TEBD RAM did not match expectation (expected: %f, gotten:%f)" % (
        exact,
        estimate,
    )
...
def test_bosonic_model_DMRG():
    ...
    estimate = engine.estimate_RAM()
    assert abs(estimate - exact) < 1.0e-10, "DMRG RAM did not match expectation (expected: %d, gotten:%d)" % (
        exact,
        estimate,
    )
